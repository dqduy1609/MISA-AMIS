$(document).ready(function () {
    ...
    $('.m-custom-combobox .combobox-dropdown-btn').attr('tabindex', '-1');
    // Lưu các các combobox-data vào combobox để tiện cho việc lọc từ
    // Lấy tất cả các combobox
    let comboboxes = $('.m-custom-combobox');
    // Với mỗi combobox tìm html của thẻ con '.m-combobox-data' rồi lưu vào data('itemDataElements') của combobox ấy
...
class Combobox {
    ...
    initEvents() {
        ...
        $('.m-combobox-btn.combobox-dropdown-btn').click(this.dropdownComboboxOnClick);
        // Chọn 1 item trong combobox-data
        $('.m-custom-combobox').on('click', '.m-combobox-data .m-combobox-item', this.itemComboboxOnClick.bind(this));
        // Bắt các key <arrow-down>, <arrow-up>, <enter>
        $('.m-custom-combobox input').keydown(this.inputComboboxKeyDown.bind(this));
        // Lọc item khi nhập các kí tự chữ cái vào combobox input
        $('.m-custom-combobox input').keyup(this.inputComboboxKeyUp.bind(this));
    }
    ...
    dropdownComboboxOnClick() {
        // highlight combobox
        $(this).siblings('input.m-combobox-input').focus()
    ...
    inputComboboxKeyDown(event) {
        ...
        let comboboxData = dropdownBtn.siblings('.m-combobox-data');
        let comboboxItems = comboboxData.children();

        // Chọn item đã được hover (nếu có)
        let hoveredItem = comboboxItems.filter('.m-combobox-item-hover');
        switch (event.keyCode) {
            case 13: // enter
                if (hoveredItem.length == 1) // Khi có 1 item dược chọn
                {
                    hoveredItem.removeClass('m-combobox-item-hover');
            ...
            case 40: // arrow-down
                ...
                } else // Nếu đã có item được hover trước đó thì chọn item bên dưới
                {
                    ...
                    hoveredItem.addClass('m-combobox-item-hover');
                }
                break;
            case 38: // arrow-up
                if (hoveredItem.length == 0) // Chưa có item nào được chọn
                {
                    ...
                    $(comboboxItems[comboboxItems.length - 1]).addClass('m-combobox-item-hover');
    ...
    inputComboboxKeyUp(event) {
        ...
        switch (event.keyCode) {
            ...
            default:
                ...
                // 2. Build html
                $(comboboxInput).siblings('.m-combobox-data').html(itemDataElements);

                // 3. Lấy tất cả item của combobox
                let items = $(comboboxInput).siblings('.m-combobox-data').children();
                ...
                $(comboboxInput).siblings('.m-combobox-data').show();
                // Tạo hiệu ứng xổ xuống cho nút dropdown
    ...
    select() {
        ...
        let combobox = $(item).parents('.m-custom-combobox');
